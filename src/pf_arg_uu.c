...
/*   Updated: 2019/03/29 18:20:46 by psentilh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_flag_uu(va_list list, t_flag flag)
{
	if (flag.modif)
	{
		...
		if (ft_strcmps(flag.modif, "l") == 0)
			flag.to_print = ft_lltoa_base(va_arg(list, unsigned long long), 10, _XMIN_);
		...
		if (ft_strcmps(flag.modif, "z") == 0)
			flag.to_print = ft_ulltoa_base((unsigned short)va_arg(list, unsigned long), 10, _XMIN_);
